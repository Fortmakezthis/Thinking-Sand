# Thinking-Sand
Repo to store files for a "mod" made with KubeJS

This is made for 1.20.1 but may work with 1.21.1 idk haven't tested it.

This needs Create. Also KubeJS, obviously. And KubeJS Create for compatibility.

I just remembered it needs Mekanism. I need to add more stuff lol.

I've begun the shift toward vanilla recipes, I will begin adding more items that were used in the original recipes, such as sheets, and maybe sulfur, idk what I'll change photolithography into.

Also, once the first version is finished, I believe I will start work on a standalone mod.

Crap I forgot to mention this reworks CC: Tweaked recipes lmao-

Update: All Create recipes are gone! Just gotta add more items so I don't need to use Create and Mekanism items.

# Installation:

Just download the repo, unzip it, and place the kubejs folder in Minecraft instance folder, if it asks to replace files, replace them.
Oh yeah, remember to restart Minecraft.
